...
from spark_serverless_example.read_file import read_file
from spark_serverless_example.save_to_bq import save_file_to_bq
from utils.spark_setup import start_spark
from utils.timer_utils import timer_args
...
def run(app_name: Optional[str],
    ...
    with Timer(**timer_args('Read File From GCS')):
        stocks_df = read_file(spark=spark, file_uri=file_uri)

    with Timer(**timer_args('Write DF to Bigquery')):
        status = save_file_to_bq(stocks_df=stocks_df, table=f'{dataset_name}.{table_name}')
        if status == 0:
